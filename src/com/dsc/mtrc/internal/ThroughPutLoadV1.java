...
public class ThroughPutLoadV1 {
	...
	public Response ThroughPutLoad(JSONObject inputJsonObj) throws JSONException {
		...
		if (! inputJsonObj.has("packagename"))
		{
	           msg= "{\"result\":\"FAILED\",\"resultCode\":500,\"message\":\""  +
	        		   "packagename json tag required for this API"  +"\"}";
	        ...
	        // statement and display that in system out.
		        ...
		        { 
		        	...
		        	JSONObject s1 =  (JSONObject) tput.get(i);
                	 String LCBLD=  s1.getString("LCBLD").toString();
                	 dscmtrclcbldid=WMSBuilding ( theurl, LCBLD) ;
  
                	 if (dscmtrclcbldid != null)
                	 {		 
                		 if (insstmt == null)insstmt="";	         
                		 insstmt=insstmt+instmpt +mtrcperiodid +","+dscmtrclcbldid +","+tmperiodid +"," +
	    	                            " getdate() ,'API','','','"+mtrcnayn+"','"+
	    	                            s1.getString("THROUGHPUT_PCT_CHG").toString() +"');";
                	 }
